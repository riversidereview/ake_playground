...
EN_TO_ZH_CHAR_NAMES = {v: k for k, v in ZH_TO_EN_CHAR_NAMES.items() if k not in ("男管理员", "女管理员")}
...
def localize_dungeon_name(name: str, locale: str = "en") -> str:
    ...
    trimmed = name.strip()
    if locale == "zh":
        if trimmed in DUNGEON_NAMES_ZH:
            return DUNGEON_NAMES_ZH[trimmed]
        if ":" in trimmed:
            prefix, suffix = trimmed.split(":", 1)
            prefix_zh = DUNGEON_NAMES_ZH.get(prefix.strip(), prefix.strip())
            suffix_zh = DUNGEON_NAMES_ZH.get(suffix.strip(), suffix.strip())
        ...
        if "·" in trimmed:
            ...
            suffix_zh = DUNGEON_NAMES_ZH.get(suffix.strip(), suffix.strip())
            return f"{prefix_zh}·{suffix_zh}"
        return DUNGEON_NAMES_ZH.get(trimmed, trimmed)

    # locale == "en"
    if trimmed in DUNGEON_NAMES_EN:
        return DUNGEON_NAMES_EN[trimmed]
    if "·" in trimmed:
        prefix, suffix = trimmed.split("·", 1)
        prefix_en = DUNGEON_NAMES_EN.get(prefix.strip(), prefix.strip())
        suffix_en = DUNGEON_NAMES_EN.get(suffix.strip(), suffix.strip())
    ...
    if " · " in trimmed:
        ...
        suffix_en = DUNGEON_NAMES_EN.get(suffix.strip(), suffix.strip())
        return f"{prefix_en} · {suffix_en}"
    return DUNGEON_NAMES_EN.get(trimmed, trimmed)
...
def friendly_name(raw, locale=None):
    ...
    if loc == "zh" and raw in EN_TO_ZH_CHAR_NAMES:
        return EN_TO_ZH_CHAR_NAMES[raw]
    return short
